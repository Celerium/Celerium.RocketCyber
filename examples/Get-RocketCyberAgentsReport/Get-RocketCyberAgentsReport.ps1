<#
    .SYNOPSIS
        Gets an agent report from the RocketCyber API.

    .DESCRIPTION
        The Get-RocketCyberAgentsReport script gets an agent report from the RocketCyber API.
    ...
    .PARAMETER APIEndPoint
        ...
        The default is https://api-us.rocketcyber.com/v3/account
...
#Region    [ Parameters ]
    ...
    param(
        ...
        $APIEndpoint = 'https://api-us.rocketcyber.com/v3/account',
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-RocketCyberAgentsReport'
    ...
    Try {
        ...
        If(Get-Module -ListAvailable -Name Celerium.RocketCyber) {
            Import-module Celerium.RocketCyber -ErrorAction Stop
        }
        Else {
            Install-Module Celerium.RocketCyber -Force -ErrorAction Stop
            Import-Module Celerium.RocketCyber -ErrorAction Stop
        }
    }
    Catch {
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting agents from RocketCyber account"

#Region     [ RocketCyber API ]

    #Customize the results
    Try{
        $RocketCyberAccountResults = Get-RocketCyberAccount -id $id

        $RocketCyberAgentResults = (Get-RocketCyberAgents -id $id).data | Select-Object `
                @{Name='customerName';Expression={$RocketCyberAccountResults.Name}}, `
...
#Region     [ CSV Report ]
    Try{
        If($Report -eq 'All' -or $Report -eq 'CSV') {
            ...
            $RocketCyberAgentResults | Sort-Object roleName | Select-Object $ScriptName,* | Export-Csv $CSVReport -NoTypeInformation
        }
    }
    Catch{
        Write-Host $_.Exception.Message -ForegroundColor Red -BackgroundColor Black
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML') {
            ...
            $TotalAgents = ($RocketCyberAgentResults.id).count
            $DeviceTypes = ($RocketCyberAgentResults.platform | Select-Object -Unique).count
            $ActiveAgents = ($RocketCyberAgentResults.status -eq 'Active'  ).count
            $InactiveAgents = ($RocketCyberAgentResults.status -eq 'InActive'  ).count
            $OfflineAgents = ($RocketCyberAgentResults.connectivity -eq 'offline').count
    ...
    #Region    [ Example HTML Report Data\Structure ]
        ...
        $TableResults = $RocketCyberAgentResults | ConvertTo-Html -As Table -Fragment -Property HostName,Status,Connectivity,Platform,agentVersion,lastConnected `
                                        -PostContent    '   <ul>
