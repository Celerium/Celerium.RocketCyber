<#
    ...
    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\BaseUri\Remove-RocketCyberBaseUri.Tests.ps1

        Runs a pester test and outputs simple results

    .EXAMPLE
        Invoke-Pester -Path .\Tests\Private\BaseUri\Remove-RocketCyberBaseUri.Tests.ps1 -Output Detailed
...
Describe "Testing [ $CommandName ] function with [ $PesterTestName ]" -Tag @('BaseUri') {
    ...
    Context "[ $CommandName ] testing function" {
        ...
        It "If the BaseUri is already gone a warning should be thrown" {
            Add-RocketCyberBaseUri
            Remove-RocketCyberBaseUri
            Remove-RocketCyberBaseUri -WarningAction SilentlyContinue -WarningVariable BaseUriWarning
